...
token make_token ( int T, pair<int, int> P) {
    token res;
    res.time_in = T;
    res.floor = P.first;
    res.direction = P.second;
    return res;
}
...
struct  floor { // структура под этаж
    ...
    static void send_task( pair<int, int> T) { // отправить запрос
        ...
        task_manager.insert( make_token( current_time(), T) );
...
struct elevator { // сам лифт
    ...
    void try_close_doors (const string& next_conditional ) {
        if ( !flag_open_door ) {
    ...
    void output_one_person (people* P) {
        try_open_doors();
        people_in.erase(P);
        sleep( T_out );
    ...
    void output () {
        ...
        for ( auto it : people_in )
            if ( it->floor_out == floor )
                Q_out.push( it );
        while ( ! Q_out.empty() ) {
            output_one_person( Q_out.front() );
            Q_out.pop();
        }
        floor_stop.erase( floor );
    }

    void input_one_person ( people* P ) {
        try_open_doors();
        sleep( T_in );
        people_in.insert( P );
    ...
    void pick_up ( struct floor* F, int direction ) { // процедура разгрузки
        ...
        output(); // выпускаем всех кого надо

        while ( people_in.size() < C ) { //запускаем всех с нужным направлением, пока не переполнимся
            if ( direction > 0 ) {
                if ( F->UP->empty() )
                    break;
...
void thread_elevator ( int n ) {
    while ( true ) {
        ...
        if ( El[n]->conditional == "waiting" ) {
            ...
            if ( El[n]->time_start_waiting + T_idle <= current_time() )
                El[n]->try_close_doors("sleeping");
            continue;
        }
...
int search_waiting ( pair<int, int> P ) { // ищем подходящий ждущий лифт
    for ( int i = 0; i < K; ++i )
...
int search_moving ( pair<int, int> P ) {  // если лифт проходит как раз в нужном направлении
    for ( int i = 0; i < K; ++i )
...
int search_sleeping ( pair<int, int> P ) { // ищем среди спящих
    int res = -1;
    int min_delta = N +2 ;
    for ( int i = 0; i < K; ++i )
...
void thread_manager () { // поток менеджера
    while ( true ) {
        guard_control_sum.lock();
        guard_task_manager.lock();
        if ( control_sum == 2 * N && task_manager.empty()) { //все люди разъехались
            guard_control_sum.unlock();
            guard_task_manager.unlock();
            for ( int i = 0; i < K; ++i )  // приказываем лифтам завершится
                El[i]->flag_process_finish = true;
            return; //кончиились задаания
        }
        else {
            guard_control_sum.unlock();
            ...
            if ( current_task.empty() )
                ...
                for ( auto to : current_task ) {
                    ...
                    int flag = search_waiting( task);
                    if ( flag != -1 ) {
                        give_task( flag, task, to );
                        continue;
                    }
                    flag = search_moving( task);
                    if ( flag != -1 ) {
                        give_task( flag, task, to);
                        continue;
                    }
                    flag = search_sleeping( task );
                    if ( flag != -1 ) {
                        give_task( flag,  task, to );
                        continue;
                    }
...
cin>>T_stage >> T_open >> T_idle >> T_close >> T_in >> T_out;
    ...
    for ( int i = 0; i < K; ++i)
        th_elevator.emplace_back( thread_elevator, i );

    thread th_manager = thread( thread_manager );
